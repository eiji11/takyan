...
#define PROPERTY_NORMALMAPFRAME "normalMapSpriteFrame"
...
void SpriteLoader::onHandlePropTypeSpriteFrame(Node * pNode, Node * pParent, const char * pPropertyName, SpriteFrame * pSpriteFrame, CCBReader * ccbReader) {
    ...
    else if (strcmp(pPropertyName, PROPERTY_NORMALMAPFRAME) == 0)
    {
        // !!!: no normal map in cocos2d-x yet
//        if(pSpriteFrame != NULL) {
//            ((Sprite *)pNode)->setSpriteFrame(pSpriteFrame);
//        } else {
//            CCLOG("ERROR: NormalMapSpriteFrame NULL");
//        }
    }
    else {
