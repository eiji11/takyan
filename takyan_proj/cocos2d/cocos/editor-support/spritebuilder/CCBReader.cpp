    ...
    if (fullPath.length() > 0)
    {
        ...
        if (!dict.empty())
        {
            ...
            for (ValueVector::iterator it = frameFileList.begin(); it != frameFileList.end(); it++) {
                std::string spriteFrameFile = it->asString();
                SpriteFrameCache::getInstance()->addSpriteFramesWithFile(spriteFrameFile);
            }
...
Scene* CCBReader::createSceneWithNodeGraphFromFile(const char *pCCBFileName, bool createPhysicsWorld)
{
    return createSceneWithNodeGraphFromFile(pCCBFileName, nullptr, createPhysicsWorld);
}

Scene* CCBReader::createSceneWithNodeGraphFromFile(const char *pCCBFileName, Ref *pOwner, bool createPhysicsWorld)
{
    return createSceneWithNodeGraphFromFile(pCCBFileName, pOwner, Director::getInstance()->getWinSize(), createPhysicsWorld);
}

Scene* CCBReader::createSceneWithNodeGraphFromFile(const char *pCCBFileName, Ref *pOwner, const Size &parentSize, bool createPhysicsWorld)
{
    Node *pNode = readNodeGraphFromFile(pCCBFileName, pOwner, parentSize);
    Scene *pScene;
    if (createPhysicsWorld)
    {
        pScene = Scene::createWithPhysics();
    }
    else
