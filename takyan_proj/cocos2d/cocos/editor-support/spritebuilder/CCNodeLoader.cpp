...
// #endif
        */
        ...
        switch(type)
        {
            ...
            {
                Color4F color4 = this->parsePropTypeColor4(pNode, pParent, ccbReader, propertyName.c_str());
                if(setProp) 
                {
            ...
            case CCBReader::PropertyType::BLOCK:
            {
                ...
                if (dynamic_cast<ControlLoader*>(this)) {
...
//                }
            ...
            case CCBReader::PropertyType::EFFECTS:
                this->readEffects(pNode, pParent, ccbReader);
                break;
            default:
    ...
    this->onDone(pNode, pParent, ccbReader);
}
...
void NodeLoader::readEffects(cocos2d::Node * pNode, cocos2d::Node * pParent, CCBReader * ccbReader)
{
    int numberOfEffects = ccbReader->readIntWithSign(false);
    ...
    cocos2d::Vector<Node*> effects;
    
    for (int i = 0; i < numberOfEffects; i++)
    {
        std::string className = ccbReader->readCachedString();
        
        CCBReader::TargetType memberVarAssignmentType = static_cast<CCBReader::TargetType>(ccbReader->readIntWithSign(false));
        std::string memberVarAssignmentName;
        if(memberVarAssignmentType != CCBReader::TargetType::NONE)
        {
            memberVarAssignmentName = ccbReader->readCachedString();
        }
        
        NodeLoaderLibrary *library = NodeLoaderLibrary::getInstance();
        
        NodeLoader *ccNodeLoader = library->getNodeLoader(className.c_str());
        ...
        Node *node = ccNodeLoader->loadNode(NULL, ccbReader);
        
        
        int propCount = ccbReader->readIntWithSign(false);
        
        
        for(int propIndex = 0; propIndex < propCount; propIndex++)
        {
            this->parseProperties(node, NULL, ccbReader);
        }
...
//            spriteFile = getFullResourcePath(spriteFile.c_str());
            bool exists = FileUtils::getInstance()->isFileExist(spriteFile.c_str());
...
Color3B NodeLoader::parsePropTypeColor3(Node * pNode, Node * pParent, CCBReader * ccbReader, const char *pPropertyName) {
    ...
    cocos2d::Color4F NodeLoader::parsePropTypeColor4(cocos2d::Node * pNode, cocos2d::Node * pParent, CCBReader * ccbReader, const char *pPropertyName) {
        
    float r = ccbReader->readFloat();
    float g = ccbReader->readFloat();
    float b = ccbReader->readFloat();
    float a = ccbReader->readFloat();
...
//DuongNT
//        pNode->setGlobalZOrder(float globalZOrder)
    } else {
        ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
    }
}
...
void NodeLoader::onHandlePropTypeColor4(Node * pNode, Node * pParent, const char* pPropertyName, Color4F pColor4F, CCBReader * ccbReader) {
    ASSERT_FAIL_UNEXPECTED_PROPERTY(pPropertyName);
}
...
void NodeLoader::onDone(cocos2d::Node * pNode, cocos2d::Node * pParent, CCBReader * ccbReader) {
